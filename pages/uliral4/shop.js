        ...
        <div className="w-full h-screen flex">
            ...
            <div className="w-4/5 h-full bg-slate-200 overflow-auto p-4 flex flex-col space-y-4">
                <div className="w-full h-12 flex justify-between bg-slate-200">
                    <input value={search} onChange={(Search) => setSearch(Search.target.value)} className="px-6 py-3 focus:outline-none rounded-xl text-black" placeholder="Search..."></input>
                    <div className="px-6 py-3 bg-orange-600 hover:bg-orange-800 cursor-pointer rounded-xl flex justify-center items-center text-white">Add Item</div>
                </div>
                {filteredStock.length > 0 ? filteredStock.map((item) => (
                    <div key={item.id} className="w-full h-32 rounded-xl bg-white flex items-center justify-between px-4">
                        <div className="flex space-x-4 items-center text-black">
                            <img className="w-24 h-24 rounded-full border-2 border-slate-400 object-contain" src={item.imgsrc}></img>
                            <h1>{item.name}</h1>
                            <h1>{item.price}</h1>
                            <h1>{item.brand}</h1>
                        </div>
                        <div className="flex space-x-8 items-center text-black">
                            <div className="flex flex-col space-y-2 text-black items-end">
                            ...
                            <div className="px-8 py-4 bg-orange-600 hover:bg-orange-800 cursor-pointer rounded-xl flex justify-center items-center text-white">Add to Cart</div>
                        </div>
                    </div>
